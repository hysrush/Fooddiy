...
<body>
					...
					<div class="col-md-3">
						<aside class="sidebar" id="sidebar" data-plugin-sticky
							...
							<ul class="nav nav-list mb-xlg">
								<li><a
									href="${ pageContext.request.contextPath }/store/findStore.do">매장
										찾기</a></li>
								<li class="active"><a
									href="${ pageContext.request.contextPath }/store/myStore.do">근처
										매장</a></li>
							</ul>
						</aside>
					</div>
						...
						<div class="tabs tabs-bottom tabs-center tabs-simple">
							<ul class="nav nav-tabs">
								<li class="active"><a href="#tabsNavigationSimple1"
									data-toggle="tab" aria-expanded="true">매장찾기</a></li>
							</ul>
	...
	<script type="text/javascript"
		src="//dapi.kakao.com/v2/maps/sdk.js?appkey=6e4954cc0e63bd91f632decfb6b47fd4&libraries=services"></script>
	...
	<script>
		...
		var j = 0;
	        	       ...
	        	       for(var k = 0; k < storeZip.length; k++) 	
	        	        if(marker.getTitle() == storeZip[k]){
	       			   console.log("k : " + k);
				      	            ...
				      	            iwContents += storeName[k]+ '점<br/>' + storePhone[k] + '<br/>'; 
				      	            iwContents += '<input type="submit" name = "storeChoice" onclick="choice(\''+storeName[k]+'\')" value="선택" />';
				      				iwContents += '</div>';
				       	        
				       	        
				       	      //  console.log("우편번호 ㅎ: " + marker.title);
	        	...
	        	/* 
	        	geocoder.addressSearch(storeAddr[2], function(result, status) {
	        	    ...
	        	    // 정상적으로 검색이 완료됐으면 
	        	     if (status === daum.maps.services.Status.OK) {
	        	        ...
	        	        var iwContents = '';
	       	        	...
	       	        	iwContents += '<form  class="nearStore" action = "" method="POST">';		
	       	            iwContents += '<div style="width:150px;height:80px;text-align:center;padding:6px 0;">';
	       	            iwContents += storeName[2]+ '<br/>' + storePhone[2] + '<br/>'; 
	       	            iwContents += '<input type="submit" name = "storeChoice" onclick="choice(\''+storeName[2]+'\')" value="선택" />';
	       				iwContents += '</div>';
	...
	<script>
		function choice(storeName) {
			...
			$('.nearStore')
					.attr('action',
							"${pageContext.request.contextPath}/menu/select_ingredients.do") // 메뉴페이지      
			$('.nearStore').attr('method', "POST") // 메뉴페이지      
			</c:when>

			<c:otherwise>
			$('.nearStore').attr('action',
					"${pageContext.request.contextPath}/menu/cart.do") //구매페이지
